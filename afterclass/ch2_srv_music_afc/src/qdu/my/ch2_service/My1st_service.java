...
import qdu.my.ch2_service.Music_bind_srv.Music_binder;
...
public class My1st_service extends Activity{
	...
	Intent it,it2,it_nobind_music, it_bind_music;	
	Myconn mc = new Myconn();//创建链接对象，绑定时使用（作为bindservice的第二个实参）
	Music_conn music_conn = new Music_conn();
	Mybinder mb;
	Music_binder music_b;//绑定音乐服务的代理对象的指针（引用）
		...
		public void onClick(View v) {//回调函数的参数是指向当前的事件源对象	
			switch(v.getId()){ //获取事件源对象在界面中（xml）定义的id号
			    ...
			    case R.id.iv_music:
					...
					if(Music_bind_srv.music_flag_bind){
						iv_music.setImageResource(R.drawable.pause);
					    //music_flag = false;
					    if(it_bind_music != null)
					    	music_b.m_pause();
					    	//暂停
	...
	protected void onCreate(Bundle savedInstanceState) {
        ...
        //音乐服务绑定模式
        it_bind_music = new Intent(this, Music_bind_srv.class);
        bindService(it_bind_music, music_conn, Context.BIND_AUTO_CREATE);
        //获取服务按钮事件处理
        ...
        //音乐按钮事件处理
        iv_music.setOnClickListener(ls);
        //调用成员函数
        mystyle();
	}
	...
	protected void onDestroy() {
		...
		if(it_bind_music !=null)
			unbindService(music_conn);//在app界面退出时，解绑音乐服务
	}
	/**
	 * 绑定模式的链接类
	 */
	class Music_conn implements ServiceConnection{//邦定模式音乐服务连接类
		@Override
		public void onServiceConnected(ComponentName name, IBinder service) {
			music_b = (Music_binder)service;//获取代理对象
		}
	...
	class Myconn implements ServiceConnection{//绑定模式示例按钮的连接类
